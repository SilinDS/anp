...
public class Eaisto2Converter extends Converter.Factory {

    public static Eaisto2Converter create() {
        return create(new Gson());
    }

    public static Eaisto2Converter create(Gson gson) {
        return new Eaisto2Converter(gson);
    }

    private final Gson gson;

    private Eaisto2Converter(Gson gson) {
    ...
    final class GsonResponseBodyConverter<T> implements Converter<ResponseBody, T> {
      ...
      //              "<string xmlns=\"http://tempuri.org/\">","").replace("</string>","");
            ...
            if ( !temp.contains("Данные пользователя указаны неверно")) {
                ...
                temp =  temp.substring( firstSkb +1, temp.length()-10 );
                temp = temp.replace("\n", "");
